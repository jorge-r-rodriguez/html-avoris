document.addEventListener("DOMContentLoaded", () => {
    ...
    open(data, triggerButton) {
      ...
      if (window.innerWidth <= 697) {
        ...
        this.popover.style.top = `${
          triggerButtonRect.bottom + window.scrollY
        }px`;
  ...
  document.querySelectorAll(".card-see-details").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const days = card
        .querySelector(".card-description span")
        .textContent.trim();
  ...
  const filterCards = () => {
    const { destination, activity, accommodation, minPrice, maxPrice } =
      getFilterValues();
    ...
    cards.forEach((card) => {
      ...
      const priceText = card
        .querySelector(".card-price-amount")
        .textContent.replace(/[^\d]/g, "");
      const price = parseFloat(priceText);

      const matchesDestination =
        destination.length === 0 ||
        destination.some((d) => description.includes(d));
      const matchesActivity = activity.length === 0 || activity.includes(tag);
      const matchesAccommodation =
        accommodation.length === 0 ||
        accommodation.some((a) => description.includes(a));
